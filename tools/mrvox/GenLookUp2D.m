function [Dico, Geo, Params] = GenLookUp2D(ModelFileName,SeqFileName,PathOut,ServerInfo)
...
for NbFile = 1:numel(FileIn)
    ...
    if Para
        ...
        adv = 0;
        for a = 1:NbSimu
            if (100*a/NbSimu - adv >= 1)
                adv = 100*a/NbSimu;
                fprintf('%3.1f %s \n',  adv, '%');
            end
            [results{a,1}, results{a,2}, Geo{a}, parameters{a,1}] = VoxelSim2D_do_one(File,SeqFileName,a,1);
        end
    end
    ...
    for a=1:size(results,1)
        try
            ...
            tmp = struct2table(parameters{a});
            Params(a,:) = tmp.Variables;
        catch
            Error = [Error a];
        end
    end
