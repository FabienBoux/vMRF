...
% Model.geo.vasc.N = floor((Model.geo.vasc.Vf/pi) .* (Model.fov ./ Model.geo.vasc.R).^2);  % Fabien 06/02/18
% if Model.geo.vasc.N == 0, Model.geo.vasc.N = 1; end % Fabien 06/02/18
fov = Model.geo.vasc.R*sqrt(Model.geo.vasc.N*pi/Model.geo.vasc.Vf);
% fprintf([num2str(fov) ' - ' num2str(Model.geo.vasc.N) '\n']) % Fabien 06/02/18
