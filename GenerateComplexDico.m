...
for s = 2:length(FileSeq)
    ...
    filenames{s} = [datestr(now,'yyyy-mm-dd-HH:MM') '_' num2str(size(X,2)) '-samples' '_' num2str(size(Y,2)) '-parameters' '_' num2str(size(X,1)) '-signals' '.mat'];
...
filename = [datestr(now,'yyyy-mm-dd-HH:MM') '_complex_dico' '.mat'];
